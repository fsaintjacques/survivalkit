...
bool
sk_logger_drain(sk_logger_t *logger, size_t *drained, size_t maximum_drain,
    sk_error_t *error)
{
	sk_log_msg_t msg;
	...
	while ((!maximum_drain || count != maximum_drain) &&
	       ck_ring_trydequeue_mpmc_msg(&logger->ring, logger->buf, &msg)) {
		if (driver->log != NULL) {
